        ...
        Type::Path(ty) => match ty.path.get_ident() {
            Some(ident) => {
                if ident == "u8" {
                    ...
                    panic!("unhandled argument type {ident}")
                }
            },
...
pub fn define_opcodes(args: Args) -> TokenStream {
    let variants = args.opcodes.iter().map(|opcode| {
...
fn impl_constants(opcodes: &[Opcode]) -> TokenStream {
    let constants = opcodes.iter().enumerate().map(|(i, opcode)| {
...
fn impl_decode(opcodes: &[Opcode]) -> TokenStream {
    let decode_arms = opcodes.iter().map(|opcode| {
        let name = &opcode.name;
        let const_name = to_upper_snake_case(name);

        let mut match_idents = numbered_idents();
        let match_args = opcode.args.iter().map(|field| match arg_ty(&field.ty) {
            ArgTy::U8 => {
        ...
        let decode_args = opcode.args.iter().map(|field| {
...
fn impl_encode(opcodes: &[Opcode]) -> TokenStream {
    let encode_arms = opcodes.iter().map(|opcode| {
        let name = &opcode.name;
        let const_name = to_upper_snake_case(name);

        let match_args = opcode.args.iter().map(|field| {
            let name = field.ident.as_ref().unwrap();
            quote! { #name }
        });

        let encode_args = opcode.args.iter().map(|field| {
            let name = field.ident.as_ref().unwrap();
...
fn impl_len(opcodes: &[Opcode]) -> TokenStream {
    let len_arms = opcodes.iter().map(|opcode| {
        let name = &opcode.name;
        let len = opcode
            .args
            .iter()
            .map(|field| match arg_ty(&field.ty) {
                ArgTy::U8 => 1,
                ArgTy::U16 => 2,
            })
            .sum::<usize>()
...
fn impl_stack_effect(opcodes: &[Opcode]) -> TokenStream {
    let stack_effect_arms = opcodes.iter().map(|opcode| {
        let name = &opcode.name;

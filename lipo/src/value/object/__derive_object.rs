...
unsafe fn downcast_unchecked<'alloc, O: Object>(
    this: ObjectRefAny<'alloc>,
) -> ObjectRef<'alloc, O> {
    match this.downcast::<O>() {
        ...
        None => {
            ...
            // SAFETY functions from the Object's vtable must be only called with that
            // object as a receiver (first parameter of every function).
...
///   be always true.
/// - Object behind `this` must be unreachable through the VM roots and never
///   again used.
...
/// - Caller must provide correct receiver type, that is `this.is::<O>()` must
///   be always true.
pub unsafe fn debug_fmt<'alloc, O: Object>(
    this: ObjectRefAny<'alloc>,
...
/// - Caller must provide correct receiver type, that is `this.is::<O>()` must
///   be always true.
pub unsafe fn partial_eq<'alloc, O: Object>(
    this: ObjectRefAny<'alloc>,
    other: ObjectRefAny<'alloc>,
) -> Option<bool> {
    // SAFETY caller must use correct receiver type
    let this = unsafe { downcast_unchecked::<O>(this) };
...
/// [`ObjectVtable::get_type`]
///
/// # Safety
/// - Caller must provide correct receiver type, that is `this.is::<O>()` must
///   be always true.
pub unsafe fn get_type<'alloc, O: Object>(
    this: ObjectRefAny<'alloc>,
    alloc: &Alloc<'_, 'alloc>,
    ...
    <O as ObjectType<'alloc>>::get_type(&this, alloc)
}
